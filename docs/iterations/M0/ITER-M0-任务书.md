...
| 周期 | 3~5 天 |
| 前置 | 无(首个迭代) |
| 配套文档 | [技术设计](ITER-M0-技术设计.md) · [测试设计](ITER-M0-测试设计.md) · 流程见 [03-协作流程](../../03-协作流程与审核规范.md) · Sprint 见 [07-Scrum 流程](../../07-Scrum开发流程.md) |

> **实施方式:按 Sprint 阶段推进**——S1 工程骨架与 MockGame → S2 窗口发现与截屏 → S3 模板匹配与模板库 → S4 后台输入与 e2e 闭环,Story 卡在 `sprints/` 目录。开发 Agent 一次只领一个 Sprint,通过 Sprint Review 后领取下一个;本任务书的 AC-01~AC-10 分布在各 Sprint 的 SAC 中,迭代收尾时统一按 AC 核对。
